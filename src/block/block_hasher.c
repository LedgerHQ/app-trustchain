...
static void write_command_seed(const block_command_t *command, crypto_hash_t *digest) {
    ...
    // Topic(16 max) + ProtocolVersion(2) + GroupKey(33) + IV(16) + EncryptedSeed(64) + EphemeralPublicKey(33) + 2 * NumberOfFields
    uint8_t length = command->command.seed.topic_len + 2 + MEMBER_KEY_LEN + IV_LEN + MAX_ENCRYPTED_KEY_LEN + MEMBER_KEY_LEN + 2 * 6;
...
static void write_command_derive(const block_command_t *command, crypto_hash_t *digest) {
    ...
    // Compute encoded data length
    // Path(max 40) + GroupKey(33) + IV(16) + EncryptedXpriv(64) + EphemeralPublicKey(33) + 2 * NumberOfFields
    ...
    // Group key
    offset += write_tl(TLV_TYPE_PUBKEY, MEMBER_KEY_LEN, buffer + offset);
    offset += write_bytes(command->command.derive.group_public_key, MEMBER_KEY_LEN, buffer + offset);

    // IV
    offset += write_tl(TLV_TYPE_BYTES, sizeof(command->command.derive.initialization_vector), buffer + offset);
    offset += write_bytes(command->command.derive.initialization_vector, sizeof(command->command.derive.initialization_vector), buffer + offset);

    // Encrypted xpriv
    offset += write_tl(TLV_TYPE_BYTES, sizeof(command->command.derive.encrypted_xpriv), buffer + offset);
    offset += write_bytes(command->command.derive.encrypted_xpriv, sizeof(command->command.derive.encrypted_xpriv), buffer + offset);

    // Ephemeral public key
    offset += write_tl(TLV_TYPE_PUBKEY, MEMBER_KEY_LEN, buffer + offset);
    offset += write_bytes(command->command.derive.ephemeral_public_key, MEMBER_KEY_LEN, buffer + offset);
...
static void write_command_add_member(const block_command_t *command, crypto_hash_t *digest) {
    int offset = 0;
    uint8_t buffer[MAX_NAME_LEN + MEMBER_KEY_LEN + sizeof(uint32_t) + 3 * TLV_HEADER_LEN + TLV_HEADER_LEN];
...
static void write_command_publish_key(const block_command_t *command, crypto_hash_t *digest) {
    int offset = 0;
    uint8_t buffer[TLV_HEADER_LEN + IV_LEN + MAX_ENCRYPTED_KEY_LEN + MEMBER_KEY_LEN + MEMBER_KEY_LEN + 4 * TLV_HEADER_LEN];
    ...
    offset += write_bytes(command->command.publish_key.initialization_vector, IV_LEN, buffer + offset);

    // Encrypted xpriv
    offset += write_tl(TLV_TYPE_BYTES, MAX_ENCRYPTED_KEY_LEN, buffer + offset);
    offset += write_bytes(command->command.publish_key.encrypted_xpriv, MAX_ENCRYPTED_KEY_LEN, buffer + offset);

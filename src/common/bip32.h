...
/**
 * Derives a private key and chain code from a parent private key. This function only performs hardened derivation.
 * [Note: this function is not part of crypto.h for unit testing purposes]
 * 
 ...
 * @param[in]  index The index of the child private key to derive. (Note that this index will be hardened)
 * @param[out] child_private_key The result of the private key derivation. The buffer must be 32 bytes long.
 * @param[out] child_chain_code The result of the chain code derivation. The buffer must be 32 bytes long.
 * 
 ...
 * @param[in]  parent_private_key The parent private key. The private key must be 32 bytes long.
 * @param[in]  parent_chain_code The parent chain code. The chain code must be 32 bytes long.
 * @param[in]  path The path of the child private key to derive. (Note that this path will be hardened)

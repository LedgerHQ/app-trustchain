...
class Device:    
    ...
    CommandResponse =  Union[SeedCommandResponse,AddMemberCommandResponse, PublishKeyCommandResponse, EmptyCommandResponse]
    ...
    def set_trusted_member(transport:BackendInterface, member):
        ...
        transport.exchange(Device.CLA, Device.INS_SET_TRUSTED_MEMBER, Device.ParseStreamMode.Empty, Device.OutputDataMode.none, payload)
    ...
    def parse_block_header(transport:BackendInterface, header):
        ...
        response = transport.exchange(Device.CLA, Device.INS_PARSE_STREAM, Device.ParseStreamMode.BlockHeader, Device.OutputDataMode.none, header_bytes)
        return response.data
    
    
    def parseCommand(transport:BackendInterface, command, outputTrustedParam: bool = False): 
        command_bytes = bytearray(command)
        response = transport.exchange(Device.CLA, Device.INS_PARSE_STREAM, Device.ParseStreamMode.Command,  outputTrustedParam, command_bytes)
        return response.data
    ...
    def parseEmptyStream(transport:BackendInterface): 
        transport.exchange(Device.CLA, Device.INS_PARSE_STREAM, Device.ParseStreamMode.Empty, Device.OutputDataMode.none, bytearray(0))


    def signBlockHeader(transport: BackendInterface, header): 
        header_bytes = bytearray(header)
        data = transport.exchange(Device.CLA, Device.INS_SIGN_BLOCK, Device.ParseStreamMode.BlockHeader, Device.OutputDataMode.none, header_bytes)
        rapduData = data.data
    ...
    def finalizeSignature(transport:BackendInterface): 
        response = transport.exchange(Device.CLA, Device.INS_SIGN_BLOCK, Device.ParseStreamMode.Signature, Device.OutputDataMode.none, bytearray(0))
...
def inject_trusted_properties(command:Command, properties:Device.CommandResponse, secret):
    ...
    if command_type == CommandType.Seed:
        ...
        seed_command.encrypted_xpriv = Crypto.decrypt(secret, seed_properties.iv, seed_properties.xpriv)
        seed_command.ephemeral_public_key = Crypto.decrypt(secret, seed_properties.iv, seed_properties.ephemeralPublicKey)
        seed_command.initialization_vector = Crypto.decrypt(secret, seed_properties.iv, seed_properties.commandIv)
        seed_command.group_key = Crypto.decrypt(secret, seed_properties.iv, seed_properties.groupKey)
        return seed_command
        ...
        derive_command.encrypted_xpriv = Crypto.decrypt(secret, derive_properties.iv, derive_properties.xpriv)
        derive_command.ephemeral_public_key = Crypto.decrypt(secret, derive_properties.iv, derive_properties.ephemeralPublicKey)
        derive_command.initialization_vector = Crypto.decrypt(secret, derive_properties.iv, derive_properties.commandIv)
        derive_command.group_key =Crypto.decrypt(secret, derive_properties.iv, derive_properties.groupKey)
        ...
        publish_key_command.ephemeral_public_key = Crypto.decrypt(secret, publish_key_properties.iv, publish_key_properties.ephemeralPublicKey)
        publish_key_command.initialization_vector = Crypto.decrypt(secret, publish_key_properties.iv, publish_key_properties.commandIv)
        publish_key_command.encrypted_xpriv = Crypto.decrypt(secret, publish_key_properties.iv, publish_key_properties.xpriv)
